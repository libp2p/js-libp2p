...
const DEFAULT_SEND_CLOSE_WRITE_TIMEOUT = 5_000

// Time to wait for a connection to close gracefully before destroying it manually
// const CLOSE_TIMEOUT = 500

// Close the duplex if there is no activity after this long in ms
// const SOCKET_TIMEOUT = 2 * 60_000 // 2 mins

export interface HalfCloseableDuplexInit {
  /**
  ...
  /**
   * Invoked when data begins to be written
   */
  onSink?(options?: AbortOptions): void | Promise<void>
  ...
  /**
   * Invoked when a read timeout has occurred
   */
  onTimeout?(): void
...
export abstract class HalfCloseableDuplex implements Duplex<AsyncGenerator<Uint8ArrayList>, Source<Uint8ArrayList | Uint8Array>, Promise<void>> {
  ...
  private readonly onSink?: (options?: AbortOptions) => void | Promise<void>
   ...
   */
  sourcePush (data: Uint8Array | Uint8ArrayList): void {
    if (data instanceof Uint8Array) {
      data = new Uint8ArrayList(data)
    }
   ...
   */
  abstract sendData (data: Uint8ArrayList, options?: AbortOptions): void | Promise<void>
