...
describe('RSA', function () {
  ...
  before(async () => {
    key = await crypto.keys.supportedKeys.rsa.generateKeyPair(512)
  })

  it('encrypt and decrypt', () => {
    const data = uint8ArrayFromString('hello world')
    const enc = rsa.encrypt(key.public, data)
    const dec = rsa.decrypt(key, enc)
...
-----END PRIVATE KEY-----
`
      const key = await rsa.importPEM(pem, '')
      expect(key).to.exist()
...
`
      const key = await rsa.importPEM(pem, 'mypassword')
      expect(key).to.exist()
    })
