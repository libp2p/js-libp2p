// nb. this will one day be needed by the WebTransport transport too
// at that point we should consider tidying it up and moving what's
// below to @libp2p/crypto
...
export interface TLSCertificate {
  privateKey: string
  pem: string
  hash: MultihashDigest<0x12> // sha-256
  secret: string
}
...
export async function generateTLSCertificate (attrs: Array<{ shortName: string, value: string }>, options: GenerateWebTransportCertificateOptions = {}): Promise<ForgeWebTransportCertificate> {
...
export async function generateTLSCertificates (attrs: Array<{ shortName: string, value: string }>, options: GenerateWebTransportCertificateOptions[] = []): Promise<TLSCertificate[]> {
  return await Promise.all(
    options.map(async options => {
      const certificate = await generateTLSCertificate(attrs, options)
