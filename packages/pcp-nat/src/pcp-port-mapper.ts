...
import {
  dynamicExternalAddress,
  staticExternalAddress
} from './check-external-address.js'
...
export interface PCPPortMapperInit {
  gateway: Gateway
...
export interface PCPPortMapperComponents {
...
export class PCPPortMapper {
  ...
  constructor (components: PCPPortMapperComponents, init: PCPPortMapperInit) {
    this.log = components.logger.forComponent(
      `libp2p:pcp-nat:gateway:${init.gateway.id}`
    )
  ...
  private remapPorts (newExternalHost: string): void {
    ...
    ] of this.mappedPorts.entries()) {
      const [host, port, transport] = key.split('-')

      this.addressManager.removePublicAddressMapping(
        host,
        parseInt(port),
        externalHost,
        externalPort,
  ...
  async mapIpAddresses (options?: MapPortsOptions): Promise<void> {
    try {
      ...
      // haven't mapped yet
      const addresses = this.getUnmappedAddresses(
      ...
      for (const addr of addresses) {
        ...
        try {
          ...
          this.log(
            'created mapping of %s:%s to %s:%s for protocol %s',
            mapping.internalHost,
            mapping.internalPort,
            mapping.externalHost,
            mapping.externalPort,
          ...
          if (options?.autoConfirmAddress === true) {
            const ma = multiaddr(
              `/ip${isIPv4(mapping.externalHost) ? 4 : 6}/${mapping.externalHost}/${transport}/${mapping.externalPort}`
            )
          ...
          this.log.error(
            'failed to create mapping for %s:%d for protocol - %e',
            host,
            port,
            transport,
            err
  ...
  private assertNotBehindDoubleNAT (publicIp: string): void {
    ...
    if (isPrivate === true) {
      throw new DoubleNATError(
        `${publicIp} is private - please init PCP with 'externalAddress' set to an externally routable IP or ensure you are not behind a double NAT`
      )
    }
