...
export class YamuxMuxer implements StreamMuxer {
  ...
  private readonly onIncomingStream?: (stream: RawStream) => void
  private readonly onStreamEnd?: (stream: RawStream) => void
    ...
    this.source = pushable({
      onEnd: (err?: Error): void => {
        ...
        if (err == null) {
          void this.close()
            .catch(err => {
              this.log?.error('error closing muxer', err)
            })
        } else {
          this.abort(err)
        }
  ...
  async close (): Promise<void> {
    ...
    this.log?.trace('muxer close reason=%s', GoAwayCode.NormalTermination)

    // If err is provided, abort all underlying streams, else close all underlying streams
    await Promise.all(
      [...this._streams.values()].map(async stream => stream.close())
    )

    // send reason to the other side, allow the other side to close gracefully
    this.sendGoAway(GoAwayCode.NormalTermination)
  ...
  abort (err: Error): void {
    this.log?.error('muxer close reason=%s error=%s', GoAwayCode.InternalError, err)
    ...
    // send reason to the other side, allow the other side to close gracefully
    this.sendGoAway(GoAwayCode.InternalError)
  ...
  private _newStream (id: number, name: string | undefined, state: StreamState, direction: 'inbound' | 'outbound'): YamuxStream {
    ...
    const stream = new YamuxStream({
      ...
      // yamux handles data chunking itself
