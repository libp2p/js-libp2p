...
describe('SDP creation', () => {
  it('handles simple blue sky easily enough', async () => {
    return
    const ma = multiaddr('/ip4/192.168.0.152/udp/2345/webrtc/certhash/uEiC5LhHPI__aMbu7XAqd2Q4gB-K7YS8flM_lLg4FXE6KiA')
    const ufrag = 'MyUserFragment'
    const sdp = underTest.fromMultiAddr(ma, ufrag)
  ...
  it('extracts certhash', () => {
    ...
    const mbdecoded = mbdecoder.decode(c)
...
describe('SDP munging', () => {
  it('does a simple replacement', () => {
    const result = underTest.munge({ type: 'answer', sdp: an_sdp }, 'someotheruserfragmentstring')
