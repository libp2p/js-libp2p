...
The `peerIdFromString` function used to be quite fleixble in that it would accept
both PeerIDs encoded as CIDs, and Peer IDs encoded as multihashs.

This flexibility is no longer present and it only accepts multihash encoded
PeerIDs.

To decode a CID encoded PeerID, use the `peerIdFromCID` method:
